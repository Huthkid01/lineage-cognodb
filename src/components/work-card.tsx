...
export function WorkCard({ work }: { work: ArtworkCard }) {
  return (
    <Link
      href={`/works/${work.id}`}
      className="group flex flex-col overflow-hidden rounded-xl border border-line bg-white/50 shadow-[0_1px_0_rgba(26,21,16,0.04)] transition duration-200 hover:-translate-y-1 hover:border-ink/18 hover:shadow-[0_22px_44px_-28px_rgba(26,21,16,0.5)] sm:rounded-2xl"
    >
      ...
      <div className="flex flex-1 flex-col gap-0.5 px-2.5 py-3 sm:gap-1 sm:px-4 sm:py-4">
        <p className="font-serif text-[0.95rem] leading-snug transition-colors group-hover:text-copper sm:text-[1.15rem]">
          {work.title}
        </p>
        <p className="text-xs text-muted sm:text-sm">{work.artistName}</p>
        <p className="mt-auto pt-2 text-[10px] uppercase tracking-[0.14em] text-muted sm:pt-3 sm:text-[11px] sm:tracking-[0.16em]">
          {work.medium}
        </p>
      </div>
    </Link>
